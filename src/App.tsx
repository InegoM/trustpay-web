...
function AppContent() {
  ...
  const protectedDecisionViews: View[] = ["confirm-approval", "request-changes", "raise-dispute"];
  const visibleView =
    !canDecide && protectedDecisionViews.includes(view) ? "milestone-review" : view;

  return (
    <Layout currentView={visibleView} navigate={navigate}>
      {syncStatus === "loading" && (
        <div
          className="mb-5 rounded-xl border border-brand-mid bg-brand-light px-4 py-3 text-sm text-ink-dim"
          role="status"
        >
      ...
      {syncStatus === "error" && (
        <div
          className="mb-5 flex items-center justify-between gap-4 rounded-xl border border-danger/25 bg-danger-light px-4 py-3"
          role="alert"
        >
          <div>
      ...
      {visibleView === "milestone-approved" && <MilestoneApproved navigate={navigate} />}
      {visibleView === "request-changes" && <RequestChanges navigate={navigate} />}
      {visibleView === "request-changes-result" && <RequestChanges navigate={navigate} />}
