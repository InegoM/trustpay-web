  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className={`${CARD} p-6`}>
        <h2 style={{ fontFamily: "var(--font-display)" }} className="text-base font-semibold text-ink mb-5">
          Approval summary
        </h2>
        ...
        <div className="space-y-3">
          {[
            ...
            { label: "Milestone", value: `Milestone ${m.id} of ${PROJECT.milestones.length}: ${m.name}` },
            ...
            <div key={row.label} className="flex justify-between gap-4 py-2 border-b border-edge last:border-0">
              ...
              <span
                className={`text-sm font-medium text-ink text-right ${row.label === "Milestone value" ? "font-semibold text-base" : ""}`}
      ...
      <div className={`${CARD} p-6`}>
        ...
        <ul className="space-y-2">
          {m.submittedEvidence?.map((ev) => (
            <li key={ev.id} className="flex items-center gap-3 text-sm py-1.5 px-3 rounded-xl bg-edge/40">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
                <path d="M2 7L5 10L12 4" stroke="#2B9B8E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-ink-dim mt-1 leading-relaxed">
              Selecting <strong className="text-ink">Confirm approval</strong> records your decision that{" "}
              <strong className="text-ink">Milestone {m.id}: {m.name}</strong> meets the agreed acceptance criteria.
              This decision is logged with your identity, the timestamp, and a reference number.
              Any payment associated with this milestone is handled externally.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Checkbox */}
      <div
        className={`${CARD} p-5 cursor-pointer ${checked ? "border-brand" : ""} transition-all`}
      ...
      >
        <label className="flex items-start gap-4 cursor-pointer select-none">
          ...
          >
            {checked && (
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path d="M1.5 6L4.5 9L10.5 3" stroke="white" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            )}
          </div>
          <input
          ...
          <p className="text-sm text-ink leading-relaxed">
            I confirm that I am authorized to approve this milestone and that the submitted work meets the
            agreed acceptance criteria as defined in Agreement {PROJECT.agreementVersion}.
          </p>
        </label>
      </div>
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <button
          onClick={() => {
            if (checked) navigate("milestone-approved");
          }}
          disabled={!checked}
