  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-muted text-sm mt-1">
              ...
              Agreement {PROJECT.agreementVersion} accepted {PROJECT.agreementAccepted}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Agreed project value", value: fmt(PROJECT.agreedValue), note: "Total agreed between parties", color: "text-ink" },
          { label: "Approved milestone value", value: fmt(PROJECT.approvedValue), note: "Decisions recorded by customer", color: "text-brand" },
          { label: "Outstanding milestone value", value: fmt(PROJECT.outstandingValue), note: "Milestones not yet approved", color: "text-warn" },
        ].map((t) => (
          <div key={t.label} className={`${CARD} p-5`}>
            <p className="text-muted text-xs font-medium uppercase tracking-wider">{t.label}</p>
            <p style={{ fontFamily: "var(--font-display)" }} className={`text-2xl font-bold mt-3 ${t.color}`}>
              {t.value}
            </p>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className={`${CARD} col-span-2 p-6`}>
          <h2 style={{ fontFamily: "var(--font-display)" }} className="text-base font-semibold text-ink mb-6">
            Milestones
          </h2>
            ...
            <div className="space-y-6">
              {PROJECT.milestones.map((m, i) => (
                <div key={m.id} className="relative flex gap-5">
                  ...
                  <div className={`relative z-10 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center shadow-sm ${
                    ...
                    {m.status === "approved" ? (
                      ...
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        ...
                        <path d="M7 4.5V7L9 8.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" />
                      </svg>
                    ) : (
                  ...
                  {/* Milestone content */}
                  <div className={`flex-1 pb-6 ${i === PROJECT.milestones.length - 1 ? "pb-0" : ""}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 flex-wrap">
                          <span style={{ fontFamily: "var(--font-display)" }} className="font-semibold text-ink text-sm">
                            Milestone {m.id}: {m.name}
                          </span>
                          <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${milestoneStatusStyle[m.status]}`}>
                            {milestoneStatusLabel[m.status]}
                          </span>
                        </div>
                        <p style={{ fontFamily: "var(--font-display)" }} className="text-sm font-semibold text-ink-dim mt-1">
                          {fmt(m.value)}
                        </p>
        ...
        <div className="space-y-4">
          ...
          <div className={`${CARD} p-5`}>
            <h3 style={{ fontFamily: "var(--font-display)" }} className="text-sm font-semibold text-ink mb-3">
              Agreement details
            </h3>
            <div className="space-y-2.5">
              {[
                { label: "Version", value: PROJECT.agreementVersion },
                { label: "Accepted", value: PROJECT.agreementAccepted },
                ...
                <div key={e.id} className="flex gap-2.5 items-start">
                  <div
                    className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                      e.actorType === "customer" ? "bg-brand" : e.actorType === "system" ? "bg-muted" : "bg-ink"
                    }`}
                    aria-hidden="true"
                  />
                  <div>
