        ...
        <section className="hidden md:flex min-h-[650px] flex-col justify-between bg-ink p-10 text-white relative overflow-hidden">
          ...
          <div className="relative">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-mid">
              Milestone confidence for SMEs
            </p>
            <h1
              className="mt-4 max-w-md text-4xl font-bold leading-tight"
              style={{ fontFamily: "var(--font-display)" }}
            >
              Agreements, evidence and decisions in one trusted record.
            </h1>
            <p className="mt-5 max-w-md text-sm leading-7 text-white/65">
              TrustPay helps businesses and customers agree work, review milestone evidence and
              record decisions without claiming to hold or transfer funds.
            </p>
          </div>
          <p className="relative text-xs text-white/45">
            Protected organization access · Auditable decisions · Simulated payment records
          </p>
        </section>
        ...
        <section className="p-7 sm:p-10 md:p-12 flex flex-col justify-center">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-muted">
              {mode === "login"
                ? "Log in with your TrustPay account."
                : "Create your account using the invitation sent by an organization administrator."}
            </p>
          </div>
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            {mode === "login" ? (
              <label className="block">
                ...
                <input
                  ...
                  className="mt-1.5 w-full rounded-xl border border-edge bg-white px-3.5 py-3 text-sm text-ink"
                />
              </label>
            ) : (
              <>
            ...
            {error && (
              <div
                className="rounded-xl border border-danger/25 bg-danger-light px-3.5 py-3 text-sm text-danger"
                role="alert"
              >
                {error}
              </div>
            ...
            <button
              disabled={submitting}
              className="w-full rounded-xl bg-brand px-4 py-3 text-sm font-semibold text-white hover:bg-brand/90 disabled:cursor-wait disabled:opacity-60"
            >
          ...
          {showPrototypeAccounts && mode === "login" && (
            <div className="mt-6 rounded-xl border border-edge bg-canvas p-3.5">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-muted">
                Development-only prototype accounts
              </p>
              <div className="mt-2 grid grid-cols-2 gap-2">
                <button
                  onClick={() => chooseDemo("nadia@example.test")}
                  className="rounded-lg border border-edge bg-card px-3 py-2 text-left hover:border-brand/40"
                >
                  <span className="block text-xs font-semibold text-ink">Nadia</span>
                  <span className="text-[11px] text-muted">SME owner</span>
                </button>
                <button
                  onClick={() => chooseDemo("omar@example.test")}
                  className="rounded-lg border border-edge bg-card px-3 py-2 text-left hover:border-brand/40"
                >
                  <span className="block text-xs font-semibold text-ink">Omar</span>
                  <span className="text-[11px] text-muted">Customer approver</span>
                </button>
              </div>
            </div>
          )}
          ...
          <button
            onClick={() => switchMode(mode === "login" ? "invitation" : "login")}
          ...
          <p className="mt-3 text-center text-[11px] text-muted">
            TrustPay account access is provided by an organization invitation. Contact your
            organization administrator if you need help.
          </p>
        </section>
      </div>
    </div>
