...
export default function TrustPayProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      setProjects(
        availableProjects.map((item) =>
          projectFromApi(
            item,
            item.id === project.id ? project : PROJECT,
          ),
        ),
      )
      setProject(mapped)
      ...
      setSyncStatus("error")
      setSyncError(
        error instanceof Error ? error.message : "Unable to load TrustPay data.",
      )
    }
  ...
  const submitDecision = useCallback(
    async (
      ...
      try {
        ...
        const mappedEvent = recordedEvent
          ? activityFromApi(recordedEvent, mapped.name)
          : null
        setProject(mapped)
        setLastDecision({
          ...
          ...(decision.action === "request-changes"
            ? { responseDate: decision.responseDate }
            : {}),
        })
        await loadActivity(mapped.id, mapped.name)
        setSyncStatus("connected")
  ...
  return (
    <TrustPayContext.Provider value={value}>
      {children}
    </TrustPayContext.Provider>
  )
}
