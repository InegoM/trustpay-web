...
const FolderIcon = ({ active }: { active: boolean }) => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
    <path d="M2.5 5.5C2.5 4.67 3.17 4 4 4H7L8.5 5.5H14C14.83 5.5 15.5 6.17 15.5 7V13.5C15.5 14.33 14.83 15 14 15H4C3.17 15 2.5 14.33 2.5 13.5V5.5Z" fill={active ? "#2B9B8E" : "#7A7870"} />
  </svg>
);
...
const ClockIcon = ({ active }: { active: boolean }) => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
    ...
    <path d="M9 5.5V9L11.5 11" stroke={active ? "#2B9B8E" : "#7A7870"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
...
const HelpIcon = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
    ...
    <path d="M7 7.2C7 6.09 7.9 5.2 9 5.2C10.1 5.2 11 6.09 11 7.2C11 8.1 10.4 8.87 9.5 9.15V10" stroke="#7A7870" strokeWidth="1.5" strokeLinecap="round" />
...
const GearIcon = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true">
    ...
    <path d="M9 2.5v1.5M9 14v1.5M2.5 9H4M14 9h1.5M4.22 4.22l1.06 1.06M12.72 12.72l1.06 1.06M4.22 13.78l1.06-1.06M12.72 5.28l1.06-1.06" stroke="#7A7870" strokeWidth="1.5" strokeLinecap="round" />
  </svg>
);

const BellIcon = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true">
    <path d="M10 2.5C7.24 2.5 5 4.74 5 7.5V11L3.5 13.5h13L15 11V7.5C15 4.74 12.76 2.5 10 2.5z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
...
export default function Layout({ children, currentView, navigate }: LayoutProps) {
  ...
  const navBtn = (
    label: string,
    target: "overview" | "projects" | "activity",
    icon: ReactNode
  ) => {
    ...
    <div className="flex h-screen bg-canvas overflow-hidden" style={{ fontFamily: "var(--font-body)" }}>
          ...
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-brand flex items-center justify-center flex-shrink-0">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
                <path d="M2 4.5L7 2L12 4.5V7C12 10 7 12.5 7 12.5C7 12.5 2 10 2 7V4.5Z" fill="white" />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-card border-b border-edge flex items-center justify-between px-8 flex-shrink-0">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm">
            {crumbs.map((part, i) => (
              <span key={i} className="flex items-center gap-2">
                {i > 0 && (
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
                    <path d="M5 3L9 7L5 11" stroke="#7A7870" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                )}
                <span
                  className={
          ...
          <div className="flex items-center gap-2">
            <button className="relative p-2 rounded-xl hover:bg-edge/60 transition-all text-muted hover:text-ink" aria-label="Notifications">
              <BellIcon />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-warn rounded-full border-2 border-white" aria-hidden="true"></span>
            </button>
            <div
